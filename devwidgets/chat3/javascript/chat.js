...
sakai.chat3 = function(tuid, placement, showSettings){
	 ...
	 */
	sakai.chat3.loadChatTextInitial = function(){};
	 ...
	 *  needs to be loaded
	 */
	sakai.chat3.loadChatTextInitial = function(initial, specialjson, hasNew){
						...
						if(njson.hasOwnProperty(i)){
							...
							if ($(chatWith + "_" + i).length > 0) {
								...
								if (cont){
									...
									sakai.chat3.loadChatTextInitial(true, newactivewindows, true);
	...
	var loadPersistence = function(){
		...
		sakai.chat3.loadChatTextInitial(true);
	};
	...
	var checkOnline = function(){
		...
		$.ajax({
			url: "/_user/presence.contacts.json",
			cache: false,
			success: function(data){
...
sdata.widgets.WidgetLoader.informOnLoad("chat3");